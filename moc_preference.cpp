/****************************************************************************
** Meta object code from reading C++ file 'preference.h'
**
** Created: Sun Jul 11 21:50:28 2010
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.3)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "preference.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'preference.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.3. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Preference[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      12,   11,   11,   11, 0x0a,
      35,   29,   11,   11, 0x0a,
      66,   11,   11,   11, 0x0a,
      80,   11,   11,   11, 0x0a,
      93,   11,   11,   11, 0x0a,
     110,  106,   11,   11, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_Preference[] = {
    "Preference\0\0enregistrement()\0index\0"
    "theme_change(QListWidgetItem*)\0"
    "ouvrirproxy()\0ajoutproxy()\0supprproxy()\0"
    "pos\0selectproxy(int)\0"
};

const QMetaObject Preference::staticMetaObject = {
    { &QDialog::staticMetaObject, qt_meta_stringdata_Preference,
      qt_meta_data_Preference, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &Preference::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *Preference::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *Preference::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Preference))
        return static_cast<void*>(const_cast< Preference*>(this));
    return QDialog::qt_metacast(_clname);
}

int Preference::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QDialog::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: enregistrement(); break;
        case 1: theme_change((*reinterpret_cast< QListWidgetItem*(*)>(_a[1]))); break;
        case 2: ouvrirproxy(); break;
        case 3: ajoutproxy(); break;
        case 4: supprproxy(); break;
        case 5: selectproxy((*reinterpret_cast< int(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
